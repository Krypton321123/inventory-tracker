...
function StatCard({ title, value, sub, accent, icon }: {
  ...
  return (
    <div className="rounded-2xl p-4 sm:p-5 min-w-0" style={{ background: "#fff", border: "1px solid var(--border)" }}>
      ...
      <p className="text-xl sm:text-2xl font-semibold truncate" style={{ color: "var(--text-primary)", letterSpacing: "-0.02em" }}>{value}</p>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-xl sm:text-2xl font-semibold" style={{ color: "var(--text-primary)", letterSpacing: "-0.02em" }}>Dashboard</h1>
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-24 sm:h-28 rounded-2xl animate-pulse" style={{ background: "var(--surface-2)" }} />
          ))}
        </div>
      ) : stats ? (
        <>
